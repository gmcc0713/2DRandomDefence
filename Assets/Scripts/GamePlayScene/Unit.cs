...
public class Unit : MonoBehaviour,IPoolingObject
{
    ...
    [SerializeField] private Attack_Distance_Type attackDistanceType;    //공격 타입(원거리, 근거리)
    [SerializeField] private Attack_Type attackType;    //공격 타입(검,활,마법사,총)
    ...
    public float _attckDamage => defaultDamage + defaultDamage*0.1f*UpgradeManager.Instance._UpgradeType[(int)attackDistanceType]; // 기본 공격력 + 업그레이드 공격력 받아오기
